...
"""
============================================================
XIN CHAO TOI LA M4 STATUS EXE TOI LA NGUOI LAM RA GARENA FREE FIRE 
============================================================
DAY LA CONG CU BAND ACCC 7 DAY MA TOI DA TIM DUOC O HO DEN VU TRU
TOI LA NGUOI SAI DEP CHIU
"""
...
# ============================================================
#PHAN NAY LA NHAP TOKEN VA OPEN NHUNG TOI DA SUA XAI BANG API NEN DEO CAN NAY NUA BAN LA 1 CON BO 
# ============================================================


GUEST_UID = "ccbam"  # <-- Gâu Gâu Gâu
GUEST_PASSWORD = "cccccc"  # <-- Gâu Gâu Gâu

# NHAP TOKEN VIP VAO DAY
ACCESS_TOKEN = "m4 là nhà phát hành game"  # NHAP OPEN ID VIP 8 VAO DAY
OPEN_ID = "Garena Lon"       

# ============================================================
# BAN LA 1 CON BO CON BO LA BAN BAN LA  MOT CON BO 
# ============================================================

OAUTH_URL = "https://100067.connect.garena.com/oauth/guest/token/grant"
MAJOR_LOGIN_URL = "https://loginbp.ggpolarbear.com/MajorLogin" #QUA OB54 DEO DOI THI DEO XAI DUOC 
...
# ============================================================
# CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC CAC 
# ============================================================
...
# ============================================================
# GEMINI NGOAI TINH VOI CHAT GTP
# ============================================================
...
class Protocol:
    ...
    def build_major_login(open_id, access_token):
        ...
        fields = {
            ...
            4: "free fire",
            5: 4, #4 trong login là thuộc server mình oke
            7: "1.123.1", # nâng cấp theo game ở đăng nhập và ở game có ở góc trái
            8:"SM-A515F", # máy đéo gì cũng được t iPhone 17 pro max
            9: "Handheld", 
            10: "Verizon",
            11: "WIFI", #5G cũng được oke 
            12: 1920,
            13: 1080,
            14: "280",
            15: "ARM64-v8a",
            ...
            41: "Verizon",
            42: "WIFI",
            57: "com.dts.freefireth", #ffmax thì vứt code
            60: 36235,
            61: 31335,
            62: 2519,
            63: 703,
            ...
            79: 2,
            81: "64",
            83: "2019118695", # này đéo cần vì 1.123.1 ở trên đã có rồi nhập bừa 2019120570 
            86: "OpenGLES2",
            87: 16383,
            88: 4,
            89: b"FwQVTgUPX1UaUllDDwcWCRBpWA0FUgsvA1snWlBaO1kFYg==",
            90: random.randint(10000, 15000),
            91: "android", # server nhận mình là adroi
            92: "KqsHTymw5/5GB23YGniUYN2/q47GATrq7eFeRatf0NkwLKEMQ0PK5BKEk72dPflAxUlEBir6Vtey83XqF593qsl8hwY=", #token này có thể thay đổi vì nó là extra info nên cũng đéo cần 
            95: 110009,
            97: 1,
            98: 0,
            99: "4",
...
# ============================================================
# GARENA TUOI LON AN DUOC M4STATUS
# ============================================================
...
class FreeFireClient:
    ...
    def generate_ua():
        ...
        models = ['SM-A515F', 'Redmi 9A', 'VIVO M3', 'RMX2185', 'ASUS_Z01QD'] #['IPHONE-17' 😏
        android = ['10', '11', '12', '13']
    ...
    async def major_login(self, encrypted_payload):
        ...
        headers = {
            ...
            "X-GA": "v1 1",
            "ReleaseVersion": "OB53" #OB100 Cũng được 
        }
        ...
        try:
            async with self.session.post(MAJOR_LOGIN_URL, data=encrypted_payload, headers=headers) as resp:
                ...
                if resp.status == 200:
                    ...
                    print(f" Login Thành Công Bước 1 ({len(data)} bytes)")
                    return data
                else:
                    text = await resp.text()
    ...
    async def get_login_data(self, url, token, encrypted_payload):
        ...
        try:
            async with self.session.post(f"{url}/GetLoginData", data=encrypted_payload, headers=headers) as resp:
                if resp.status == 200:
                    data = await resp.read()
                    print(f" Thành Công Getlogin Bước 3 ({len(data)} bytes)")
                    return data
                else:
                    print(f"❌ GetLoginData HTTP {resp.status}")
        except Exception as e:
            print(f"Thất Bại Rồi : {e}")

        return None

    async def tcp_connect(self, ip, port, auth_packet, name):
        print(f"Bước Cuối Đã Band  {name}: {ip}:{port}")
        ...
        try:
            ...
            if data:
                print(f" {name} Đã Band 7 Day ({len(data)} bytes)")
                return True, writer
            else:
                writer.close()
...
class CompleteBot:
    ...
    def print_banner(self):
        print("=" * 70)
        print("TOKUDA TOKUDA TOKUDA TOKUDA TOKUDA")
        print("=" * 70)
...
#CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC CẶC 
# CODE TREN DUOC THUC HIEN BOI DEV CHAT GTP VUA AL GEMINI TUOI LON AN DUOC BO CHAT EODIBITI LA CONG CU AL TOP 2 THE GIOI CHI THUA 1 NGUOI DO CHINH LA 
#[DEV M4 STATUS EXE]
